...
# Add parent directory to path for package installation
...
try:
    ...
    def load_env(): return {}
    def get_env(key, default=None, env_map=None): return os.getenv(key, default)
    def detect_input(s): return ("url", "unknown")
    def extract_youtube_id(url): return "yt_video"
    def fetch_video_info(url, platform, token): return {}
    def _ytdlp_cmd(url, output, extra_args=None): return ["yt-dlp", "-o", output, url]
    def _build_ytdlp_cookie_args(): return []
    def _ytdlp_download(url, video_path): pass
    def _ytdlp_download_audio(url, audio_path): pass
    def download_youtube_subtitles(url, output_dir): return None
    def _vtt_to_outputs(vtt_content, srt_path, text_path): return {}
    def extract_audio(video_path, audio_path): pass
    def transcribe_sherpa_onnx(audio_path, output_dir, env_map): return {}
    def transcribe_volcengine(audio_path, output_dir, env_map): return {}
    def run(cmd, check=True, capture=False, timeout=None): pass
    def check_dep(cmd): return shutil.which(cmd) is not None
