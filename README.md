...
├── sherpa-onnx-paraformer-trilingual-zh-cantonese-en/  # 三语模型
│   ├── model.int8.onnx           # int8 量化模型 (234MB)
│   ├── model.onnx                # float32 模型 (831MB)
│   └── tokens.txt
├── sherpa-onnx-paraformer-zh-2023-03-28/               # 中文模型（备选）
└── docs/
    └── tikhub-setup.md
```
...
- **目录**：`sherpa-onnx-paraformer-trilingual-zh-cantonese-en/`
- **文件**：`model.int8.onnx` (234MB)
- **语言**：普通话 + 粤语 + 英语
- **RTF**：≈ 0.08（12x 实时）

### 中文模型（备选）

- **目录**：`sherpa-onnx-paraformer-zh-2023-03-28/`
- **文件**：`model.int8.onnx` (214MB)
- **语言**：普通话
- **RTF**：≈ 0.09
